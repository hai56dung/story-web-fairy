...
export default class StoryDetail extends BaseComponent {
  render() {
    ...
    this.generateHearts(
      !isNaN(this.props.story.avgRating) ? this.props.story.avgRating : 0,
      $avgHearts
    );
    let $avgText = document.createElement("p");
    $avgText.innerHTML = `<span class="avg-num rating-num">${
      !isNaN(this.props.story.avgRating) ? this.props.story.avgRating : 0
    }</span> average based on <span class="avg-num view-num">${
      this.props.story.usersRating.length
    }</span> reviews.`;
    ...
    $btnRating.innerHTML = `<i class="fa-solid fa-heart"></i>Rating`;
    ...
    if (auth.currentUser) {
      ...
      let ratingStar = [...$ratingStar.childNodes].filter(
    ...
    <img src="${
      this.props.story.authorImg
        ? this.props.story.authorImg
        : "https://source.unsplash.com/random/?girl"
    }" alt="" class="detail-author-image" />
