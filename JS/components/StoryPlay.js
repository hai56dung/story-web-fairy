...
export default class StoryPlay extends BaseComponent {
  render() {
    ...
    $audio.addEventListener("timeupdate", (e) => {
      ...
      let durationStr =
        duration < 10
          ? `0:0${Math.round(duration)}`
          : `0:${Math.round(duration)}`;
      let currentTimeStr =
        currentTime < 10
          ? `0:0${Math.round(currentTime)}`
    ...
    for (let i = 0; i < parseInt(story.pagesNum) + 1; i++) {
      if (i == parseInt(story.pagesNum)) {
        $pageList.innerHTML += `<img class="page" src="./DATA/${getImgByName(
          story.name
        )}/Pages/00.jpg">`;
      } else {
        $pageList.innerHTML += `<img class="page" src="./DATA/${getImgByName(
          story.name
        )}/Pages/${("0" + i).slice(-2)}.jpg">`;
      }
    }
    //play the first page
    setTimeout(() => {
      ...
      pages[i].onclick = function () {
        ...
        if (this.pageNum % 2 === 0) {
          if (this.pageNum == 2) {
            ...
            audioFiles.push(
              `./DATA/${getImgByName(story.name)}/Audios/${(
                "0" +
                (this.pageNum - Math.floor(this.pageNum / 2) - 1)
              ).slice(-2)}.mp3`
            );
          }
          ...
          audioFiles.push(
            `./DATA/${getImgByName(story.name)}/Audios/${(
              "0" +
              (this.pageNum - Math.floor(this.pageNum / 2))
            ).slice(-2)}.mp3`
          );
